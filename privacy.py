...
def age_of(profile: dict) -> int | None:
    """Âge de l'enfant, calculé depuis son année de naissance.

    POURQUOI UNE ANNÉE ET PAS UN ÂGE. Un âge saisi une fois ne vieillit pas : un profil
    créé à 9 ans en avait toujours 9 deux ans plus tard, et gardait les réglages de ses
    9 ans. Ce n'était pas cosmétique — l'âge décide du niveau de vie privée par défaut,
    donc un adolescent de 15 ans encore enregistré comme en ayant 12 restait observé par
    fenêtres de cinq minutes. Personne ne voyait la dérive. Une année de naissance
    n'exige aucun entretien.

    L'objection de confidentialité est faible : connaître l'âge d'un enfant aujourd'hui
    donne déjà son année de naissance à un an près. Une année suffit à distinguer quatre
    tranches, il n'y a aucune raison de stocker une date complète.

    PRÉCISION : on renvoie l'âge atteint DANS L'ANNÉE en cours, sans tenir compte du jour
    d'anniversaire, que le produit ne demande pas. Un enfant change donc de tranche au
    plus quelques mois trop tôt, jamais trop tard — le sens qui protège.

    Repli sur l'ancien champ `age` pour un profil écrit avant ce changement.
    """
    profile = profile or {}
...
def level_of(profile: dict) -> str:
    ...
    return lvl if lvl in LEVELS else default_level_for_age(age_of(profile))
...
def age_band(age) -> str:
    ...
    Ces quatre tranches font autorité dans tout le produit. `default_level_for_age`
    ci-dessus les regroupe (détaillé pour les deux premières, résumé pour la troisième,
    minimal pour la quatrième) ; le site vitrine et la documentation doivent les reprendre
    telles quelles plutôt qu'en inventer d'autres.
    """
    try:
        age = int(age)
    except (TypeError, ValueError):
...
# pas « L'Alice ».
# La préposition élidée « d' » est traitée à part. Elle ne s'absorbe pas, le sens de la
# phrase en dépend, mais elle ne peut pas rester telle quelle : le modèle écrivant
# « l'activité d'Enfant 1 », le parent lisait « l'activité d'Alice ». On la restitue en
# « de ».
_ALIAS_RE = re.compile(
    rf"(?:\b(?P<elided>[ldLD])['’]\s*)?\b{_ALIAS_WORDS}\s*(?:n[°ºo]\s*|#\s*|number\s*)?(?P<num>\d{{1,2}})\b",
    re.IGNORECASE)
...
def restore_names(text, config: dict) -> str:
    ...
    def _sub(m):
        name = by_number.get(m.group("num"))
        if name is None:
            return m.group(0)
        # « d'Enfant 1 » → « de Alice » : sans cela, le modèle produisant l'élision
        # devant « Enfant » laissait « d'Alice ». « L'enfant 1 » reste absorbé.
        elided = m.group("elided") or ""
        if elided.lower() == "d":
            return f"{'De' if elided.isupper() else 'de'} {name}"
        return name
