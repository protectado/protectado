...
_KIDS_KEY_RE = re.compile(r"^[\x20-\x7e]{8,63}$")
_HOSTAPD_AP_CONF = "/etc/protectado/hostapd-ap.conf"


def set_kids_key(args: dict):
    """Change la clé du Wi-Fi des enfants depuis le dashboard.

    Deux endroits portent la clé et doivent rester d'accord : config.json
    (network.kids.key), relu par ap-persist.sh à chaque réinstallation de la posture,
    et la conf hostapd effectivement servie. Écrire l'un sans l'autre ferait revenir
    l'ancienne clé à la prochaine réinstallation, ou ne changerait rien tout de suite.

    Le redémarrage de hostapd coupe l'AP quelques secondes : tous les appareils des
    enfants sont déconnectés et ne reviendront qu'avec la nouvelle clé. Le parent en
    est averti dans le dashboard AVANT de valider — c'est là que l'avertissement a un
    sens, pas pendant l'installation où il n'y a encore aucun appareil connecté.
    """
    key = args.get("key") or ""
    if not _KIDS_KEY_RE.match(key):
        log.error("set_kids_key refusé — clé hors format WPA2 (8–63 caractères simples)")
        return
    try:
        with open(_CONFIG_PATH) as f:
            cfg = json.load(f)
    except Exception as e:
        log.error(f"set_kids_key : config.json illisible : {e}")
        return
    net = cfg.get("network") or {}
    if net.get("enforcement") != "gateway" or not (net.get("kids") or {}).get("ssid"):
        log.error("set_kids_key refusé — le boîtier ne diffuse pas de Wi-Fi enfants")
        return
    net["kids"]["key"] = key
    cfg["network"] = net
    try:
        ...
        log.error(f"set_kids_key : écriture config.json impossible : {e}")
        return

    # Conf hostapd : on remplace la seule ligne wpa_passphrase. Réécrire tout le fichier
    # ferait perdre le pays / la bande que le pré-démarrage y a inscrits au dernier boot.
    try:
        with open(_HOSTAPD_AP_CONF) as f:
            lines = f.read().splitlines()
        lines = [f"wpa_passphrase={key}" if l.startswith("wpa_passphrase=") else l
                 for l in lines]
        with open(_HOSTAPD_AP_CONF, "w") as f:
            f.write("\n".join(lines) + "\n")
        os.chmod(_HOSTAPD_AP_CONF, 0o600)      # contient la clé en clair
    except Exception as e:
        log.error(f"set_kids_key : {_HOSTAPD_AP_CONF} non modifié : {e}")
        return

    r = subprocess.run(["systemctl", "restart", "protectado-ap.service"],
                       capture_output=True, text=True)
    if r.returncode != 0:
        log.error(f"set_kids_key : redémarrage AP échoué — {r.stderr.strip()[:200]}")
        return
    log.info("set_kids_key : nouvelle clé appliquée, AP enfants redémarré")
...
HANDLERS = {
    ...
    "set_kids_key":      set_kids_key,        # changement de clé du Wi-Fi enfants
}
...
def main():
    ...
    while True:
        global _last_cleanup, _last_gateway_check
        now_ts = time.time()
