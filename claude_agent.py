...
_LANG_PROMPTS = {
    "fr": {
        ...
        "data": "Ne rapporte que des chiffres et des horaires présents dans les données reçues. Si la granularité fournie ne permet pas de répondre, dis ce que tu as, dis ce qui manque et pourquoi, sans estimer ni déduire. N'invente jamais une fonctionnalité du produit : si tu ne sais pas où se trouve un réglage, ne le devine pas.",
    ...
    "en": {
        ...
        "data": "Only report figures and times that appear in the data you received. If the granularity provided cannot answer the question, say what you have, say what is missing and why, without estimating or inferring. Never invent a product feature: if you do not know where a setting lives, do not guess.",
    ...
    "es": {
        ...
        "data": "Informa únicamente de cifras y horarios presentes en los datos recibidos. Si la granularidad facilitada no permite responder, di lo que tienes, di lo que falta y por qué, sin estimar ni deducir. Nunca inventes una función del producto: si no sabes dónde está un ajuste, no lo adivines.",
    ...
    "pt": {
        ...
        "data": "Relate apenas números e horários presentes nos dados recebidos. Se a granularidade fornecida não permitir responder, diga o que tem, diga o que falta e por quê, sem estimar nem deduzir. Nunca invente uma funcionalidade do produto: se não souber onde está uma configuração, não a adivinhe.",
...
def _build_system_prompt() -> str:
    try:
        ...
        for key in sorted(aliases):
            ...
            if pr.get("mode") == "monitoring":
                continue
            label = aliases[key]
            band = privacy.age_band(privacy.age_of(pr))
    ...
    return "\n".join([
        ...
        p["labels"],
        # Le modèle comblait les trous : questionné sur les horaires d'un profil dont la
        # granularité est réduite, il présentait les créneaux du planning comme des
        # horaires d'usage, et renvoyait le parent vers un écran qui n'existait pas.
        p["data"],
        _SECURITY_RULES,
    ])
...
PARENT_TOOLS = [
    ...
    },
    {
        ...
        "function": {
            "name": "query_history",
            # La description promettait « heure par heure » sans condition, alors que la
            # granularité dépend du niveau de vie privée du profil. Le modèle demandait
            # donc un détail horaire, recevait des agrégats, et fabriquait les horaires
            # manquants à partir du planning. Elle annonce maintenant les trois cas.
            "description": (
                "Analyse l'activité DNS passée d'un profil sur une date donnée, avec le "
                "mode Pi-hole actif et si le domaine aurait dû être bloqué. "
                "La granularité dépend du niveau de vie privée du profil et est indiquée "
                "par le champ « granularite » du résultat : `detailed` = fenêtres de "
                "5 minutes, `summary` = agrégats par demi-journée (aucune heure précise), "
                "`minimal` = totaux du jour (aucun horaire). "
                "Ne déduis jamais un horaire qui n'est pas dans le résultat : le planning "
                "dit ce qui était AUTORISÉ, pas ce qui a été utilisé. "
...
def _execute_parent_tool(name: str, args: dict, config: dict,
    ...
    if name == "allow_mode_for":
        ...
        def _restore():
            try:
                # Celui qui efface annonce : le cycle du monitor rattrape la même
                # échéance, et sans cette garde la fin était journalisée deux fois.
                if not scheduler.clear_temp_override(profile):
                    return
                cfg  = _load_config()
    ...
    if name == "query_history":
        ...
        if not detailed_access and level != privacy.DETAILED:
            ...
            return json.dumps({
                ...
                "schedule": schedule,
                "granularite": level,
                # Cette note annonçait un écran de consultation horaire qui n'existait
                # pas encore : le modèle la répétait au parent telle quelle. Elle décrit
                # maintenant les deux chemins réellement disponibles.
                "note": ("Granularité réduite par le niveau de vie privée du profil. "
                         "Le blocage et les alertes ne sont pas affectés. Ne complète pas "
                         "ces agrégats par des horaires déduits du planning. Deux chemins "
                         "pour le parent : l'onglet Profils, bouton « Voir le détail d'une "
                         "journée » (mot de passe demandé, une seule date, consultation "
                         "inscrite au journal des événements), ou relever le niveau de vie "
                         "privée du profil dans ce même onglet."),
                "domains": domains_out,
        ...
        result = {
            ...
            "override": override,
            "schedule": schedule,
            # Annoncé aussi quand la granularité est fine : un champ présent d'un côté
            # seulement laisse le modèle interpréter son absence.
            "granularite": privacy.DETAILED,
            ...
            "events": [
                {
                    ...
                    "domain": e["domain"],
                    "message": e["message"],
                    # Le tableau de bord traduit l'événement depuis sa clé : sans elle,
                    # le détail d'une journée restait en français pour une famille en/es/pt.
                    "message_key": e.get("message_key"),
                    "params": e.get("params"),
                }
                for e in events
            ],
...
def weekly_report() -> str:
    ...
    for pname, profile in config["profiles"].items():
        ...
        profiles_data[aliases.get(pname, pname)] = {
            "tranche_age": privacy.age_band(privacy.age_of(profile)),
