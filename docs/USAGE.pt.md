...
### Chave do Wi-Fi das crianças

Em modo gateway o equipamento emite o seu próprio Wi-Fi para as crianças. A chave altera-se
em **Gestão → Wi-Fi das crianças**: escreva uma chave nova (ou gere uma legível) e
confirme. Útil se a chave circulou para fora de casa, ou se foi esquecida: nunca volta a
ser mostrada, só pode ser substituída.

A alteração desliga **todos** os aparelhos das crianças: só voltam depois de a nova chave
ser introduzida em cada um. O Wi-Fi do router, o dos adultos, não é afetado.
...
um dia concreto: **Perfis → Modificar → Ver o detalhe de um dia**. A palavra-passe é pedida
