...
╚══════════════════════════════════════════════════╝
  ...
  Dashboard  →  http://protectado.local
             ou http://192.168.x.x  (si le nom ne répond pas)
...
**Conserver le mot de passe Pi-hole** — pour l'interface d'admin Pi-hole, sur `http://protectado.local:81`.
...
**L'assistant de configuration ne s'ouvre pas**

Tant qu'un parent ne l'a pas configuré, le boîtier sait se réparer seul. Branche un câble Ethernet entre le boîtier et la box, puis redémarre :

```bash
sudo reboot
```

Au démarrage, s'il trouve une connexion filaire, le boîtier va chercher la dernière version publiée et la réinstalle avant de diffuser le Wi-Fi `Protectado-Setup`. Compte quelques minutes : le réseau n'apparaît qu'une fois la mise à jour terminée. Pour suivre l'opération :

```bash
journalctl -b -t protectado-boot
```

Ce rattrapage ne concerne QUE les boîtiers en attente de configuration. Une fois configuré, un boîtier ne se met plus à jour de lui-même au démarrage : le filtrage d'une famille ne doit pas s'interrompre sans que personne ne l'ait décidé.
