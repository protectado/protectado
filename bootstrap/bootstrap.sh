...
run_update() {
  ...
  # Refspec explicite : cf. la note du chemin d'installation ci-dessous (clone
  # mono-branche → origin/$BRANCH inexistant pour toute autre branche).
  if repo_git fetch origin "+refs/heads/$BRANCH:refs/remotes/origin/$BRANCH" 2>&1 | tee -a "$LOG_FILE"; then
  ...
  # L'unité de l'orchestrateur de posture n'était régénérée qu'à l'installation
  # initiale : un boîtier mis à jour gardait donc indéfiniment la version du fichier
  # posée le premier jour, y compris son délai de démarrage. On la repose ici, mais
  # SEULEMENT si ce boîtier utilise déjà l'orchestrateur — l'installer sur une machine
  # qui n'en a jamais eu changerait son modèle de démarrage à son insu.
  if [ -f /etc/systemd/system/protectado-boot.service ]; then
    bash "$INSTALL_DIR/bootstrap/protectado-boot.sh" install >> "$LOG_FILE" 2>&1 \
      && ok "Orchestrateur de posture mis à jour" \
...
step3_protectado() {
  ...
  if [ -d "$INSTALL_DIR/.git" ]; then
    ...
    if repo_git fetch origin "+refs/heads/$BRANCH:refs/remotes/origin/$BRANCH" >> "$LOG_FILE" 2>&1 \
       && repo_git checkout --quiet -B "$BRANCH" >> "$LOG_FILE" 2>&1 \
  ...
  if command -v nono &>/dev/null; then
    ...
    # Version FIGÉE, et surtout PAS « la dernière ». protectado-agent.json est un profil
    # écrit contre une version précise de nono : prendre la release du jour fait installer
    # un bac à sable que ce profil n'a jamais vu. C'est exactement ce qui s'est produit —
    # la sémantique des ports a été scindée en deux clés (open_port = connexion localhost
    # sortante, listen_port = port que l'enfant peut binder), et un profil ne déclarant que
    # open_port a vu son dashboard refusé au bind sur 0.0.0.0:8080. L'agent bouclait, le
    # portail captif montait sans rien derrière, et RIEN dans le dépôt n'avait changé.
    # Monter de version est donc une décision à prendre, à tester et à commiter, jamais
    # un effet de bord de la date d'installation. Surcharge ponctuelle possible :
    #     PROTECTADO_NONO_VER=0.76.0 sudo -E bash bootstrap.sh
    NONO_VER="${PROTECTADO_NONO_VER:-0.75.0}"
  ...
  # Landlock présent ET assez récent ? L'agent tourne avec « nono --sandbox-policy
  # landlock », qui exige que le noyau porte AUSSI la politique réseau (règles par port).
  # C'est l'ABI qui le dit : il faut au moins la V4. Sans cela, le service refusera de
  # démarrer, franchement et en le disant — c'est voulu, mais autant l'annoncer ici,
  # pendant l'installation, plutôt que de le laisser découvrir au premier boot.
  if command -v nono &>/dev/null; then
    if ! grep -qw landlock /sys/kernel/security/lsm 2>/dev/null; then
      log "   ⚠ Landlock INACTIF sur ce noyau ($(uname -r)) — protectado-agent NE DÉMARRERA PAS."
      log "     Sur Raspberry Pi OS : noyau sans Landlock. Utiliser Ubuntu (linux-raspi)."
    else
      # landlock_create_ruleset(NULL, 0, LANDLOCK_CREATE_RULESET_VERSION) → numéro d'ABI.
      # Syscall 444 sur arm64 comme sur x86_64. Échec de la sonde = on ne conclut rien.
      LL_ABI=$(python3 -c 'import ctypes;print(ctypes.CDLL("libc.so.6").syscall(444,None,0,1))' 2>/dev/null || echo "")
      case "$LL_ABI" in
        ''|*[!0-9]*) : ;;                                  # sonde indisponible : on se tait
        *) if [ "$LL_ABI" -lt 4 ]; then
             log "   ⚠ Landlock ABI $LL_ABI (< 4) — pas de règles réseau par port."
             log "     protectado-agent NE DÉMARRERA PAS. Noyau plus récent requis."
           else
             ok "Landlock actif (ABI $LL_ABI) — sandbox réseau par port disponible"
           fi ;;
      esac
    fi
  fi
...
step6_summary() {
  ...
  # Adresse ANNONCÉE au parent : le nom mDNS d'abord, l'IP en repli juste dessous.
  # Le nom est ce qui survit dans le temps — une IP notée sur un bout de papier devient
  # fausse au premier renouvellement de bail DHCP, et le parent se retrouve devant un
  # tableau de bord introuvable sans comprendre pourquoi.
  # Le nom n'est toutefois annoncé QUE s'il résout réellement depuis cette machine :
  # mDNS reste capricieux selon les box et les téléphones, et une adresse annoncée qui
  # ne répond pas est pire que pas d'adresse du tout. Minuscules à l'affichage, le nom
  # d'hôte pouvant être capitalisé (mDNS, lui, est insensible à la casse).
  MDNS_NAME="$(hostname | tr '[:upper:]' '[:lower:]').local"
  if getent hosts "$MDNS_NAME" >/dev/null 2>&1 || avahi-resolve -4 -n "$MDNS_NAME" >/dev/null 2>&1; then
    DASHBOARD_HOST="$MDNS_NAME"; FALLBACK_HOST="$LOCAL_IP"
  else
    DASHBOARD_HOST="$LOCAL_IP"; FALLBACK_HOST=""
  fi
  ...
  if [ -n "$FALLBACK_HOST" ]; then
    echo "             ou http://$FALLBACK_HOST  (si le nom ne répond pas)"
  fi
  echo "  Pi-hole    →  http://$DASHBOARD_HOST:81  (admin/debug)"
  echo ""
