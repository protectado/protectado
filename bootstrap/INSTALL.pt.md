...
╚══════════════════════════════════════════════════╝
  ...
  Painel  →  http://protectado.local
          ou http://192.168.x.x  (se o nome não responder)
...
**Guarde a palavra-passe do Pi-hole** — para a interface de admin do Pi-hole, em `http://protectado.local:81`.
...
**O assistente de configuração não abre**

Enquanto nenhum dos pais o tiver configurado, o aparelho sabe reparar-se sozinho. Liga um cabo Ethernet entre o aparelho e o router e reinicia:

```bash
sudo reboot
```

No arranque, se encontrar uma ligação por cabo, o aparelho procura a última versão publicada e reinstala-a antes de emitir o Wi-Fi `Protectado-Setup`. Conta alguns minutos: a rede só aparece depois de a atualização terminar. Para acompanhar:

```bash
journalctl -b -t protectado-boot
```

Esta recuperação diz respeito APENAS a aparelhos à espera de configuração. Depois de configurado, um aparelho deixa de se atualizar sozinho no arranque: a filtragem de uma família não deve parar sem que alguém o tenha decidido.
