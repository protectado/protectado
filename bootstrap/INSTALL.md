...
╚══════════════════════════════════════════════════╝
  ...
  Dashboard  →  http://protectado.local
             or http://192.168.x.x  (if the name does not answer)
...
**Keep the Pi-hole password** — for the Pi-hole admin UI, reachable at `http://protectado.local:81`.
...
**The setup assistant does not open**

As long as no parent has configured it, the box can repair itself. Plug an Ethernet cable between the box and the router, then reboot:

```bash
sudo reboot
```

On startup, if it finds a wired connection, the box fetches the latest published version and reinstalls it before broadcasting the `Protectado-Setup` Wi-Fi. Allow a few minutes: the network only appears once the update has finished. To follow along:

```bash
journalctl -b -t protectado-boot
```

This recovery path applies ONLY to boxes still awaiting configuration. Once configured, a box no longer updates itself at startup: a family's filtering must not stop without anyone deciding so.
