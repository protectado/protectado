...
╚══════════════════════════════════════════════════╝
  ...
  Panel  →  http://protectado.local
         o http://192.168.x.x  (si el nombre no responde)
...
**Conserva la contraseña de Pi-hole** — para la interfaz de admin de Pi-hole, en `http://protectado.local:81`.
...
**El asistente de configuración no se abre**

Mientras ningún padre o madre lo haya configurado, el dispositivo sabe repararse solo. Conecta un cable Ethernet entre el dispositivo y el router y reinicia:

```bash
sudo reboot
```

Al arrancar, si encuentra una conexión por cable, el dispositivo busca la última versión publicada y la reinstala antes de emitir la Wi-Fi `Protectado-Setup`. Cuenta unos minutos: la red solo aparece cuando la actualización ha terminado. Para seguir la operación:

```bash
journalctl -b -t protectado-boot
```

Esta recuperación solo afecta a los dispositivos pendientes de configuración. Una vez configurado, un dispositivo ya no se actualiza solo al arrancar: el filtrado de una familia no debe interrumpirse sin que nadie lo haya decidido.
