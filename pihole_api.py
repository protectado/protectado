...
def is_real_device_ip(value: str) -> bool:
    """L'adresse désigne-t-elle un APPAREIL de la maison, montrable au parent ?

    FTL mélange dans ses clients et sa table réseau des adresses qui ne sont l'appareil
    de personne : 127.0.0.1 et ::1 (le boîtier se parlant à lui-même), 0.0.0.0 (requête
    sans source identifiée), ff02::16 (multicast MLD émis par la pile IPv6 du boîtier).
    Elles n'ont aucun sens pour un parent qui cherche le téléphone de son enfant, et
    proposer de leur rattacher un profil n'aurait même pas de résultat observable.

    Une valeur qui n'est pas une IP est conservée : Pi-hole accepte aussi des clients
    désignés par MAC ou par nom d'hôte, et ce n'est pas à ce filtre d'en décider.
    """
    try:
    ...
    return not (addr.is_loopback or addr.is_unspecified or addr.is_multicast
                or addr.is_reserved or value == "255.255.255.255")
...
class PiHoleAPI:
    ...
    def get_clients(self) -> list:
        # Les adresses qui ne sont l'appareil de personne (boucle locale, multicast…)
        # sont écartées ICI, à la source : tout ce qui les afficherait ensuite — page
        # Appareils, carte « nouveaux appareils », inventaire ARP — les recevrait sinon.
        data = self._get("/clients")
        return [c for c in data.get("clients", [])
                if is_real_device_ip(c.get("client") or c.get("ip") or "")]
    ...
    def get_network_devices(self) -> list[dict]:
        ...
        for dev in data.get("devices", []):
            ...
            for ip_entry in dev.get("ips", []):
                ip = ip_entry.get("ip", "")
                if ip and is_real_device_ip(ip):
                    devices.append({
                        "ip":       ip,
                        "mac":      mac,
                        "vendor":   vendor,
